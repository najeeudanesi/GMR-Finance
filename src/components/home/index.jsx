...
const Home = (props) => {
  ...
  const makePostRequest = useCallback(
    async (token, standalone) => {
      ...
      try {
        ...
        // Conditionally append isStandalone parameter
        const authUrl = standalone
          ? `/Auth/auth?AuthToken=${token}&iSStandalone=${standalone}`
          : `/Auth/auth?AuthToken=${token}`;

        const data = await post(authUrl, {
          email: email,
        });
        ...
        sessionStorage.setItem(
          "token-expiry-date",
          data?.resultList?.expirationDate
        );
        localStorage.setItem("homeLink", data?.resultList?.homeLink);

        localStorage.setItem(
          "name",
          (data?.resultList?.firstName || "user") +
            " " +
            (data?.resultList?.lastName || "name")
        );
  ...
  useEffect(() => {
    ...
    const token = params.get("emp");
    const standalone = params.get("isStandalone") === "true";
    ...
    // const isStandalone = params.get("isStandalone") === "true";
    token && makePostRequest(token, standalone);
  ...
  }, [location.search, makePostRequest]);
