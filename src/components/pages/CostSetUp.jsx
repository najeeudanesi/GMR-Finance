...
function CostSetUp() {
  ...
  const getFilteredData = async (page, size, searchText) => {
    try {
      const data = await get(`/costsetup/list/${page}/${size}`);
      const filtered = data.resultList.filter(item => {
        return item.itemName.includes(searchText);
      });
  ...
  const fetchData = async () => {
    ...
    if (searchText === "") {
      ...
      await getFilteredData(currentPage, pageSize, searchText);
    }
    setLoading(false);
  };
